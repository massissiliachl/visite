...
import { API_BASE_URL, API_USERS_URL } from '../../config/api';
...
export default function AdminDashboard() {
  ...
  const [showDatePickerClient, setShowDatePickerClient] = useState(false);
  
  // États paiement client
  ...
  const [boatCircuit, setBoatCircuit] = useState('60');
  const [boatSlot, setBoatSlot] = useState('09h-11h');
  ...
  const [boatSelectedTime, setBoatSelectedTime] = useState('');
  
  // États paiement bateau
  ...
  // États pour l'onglet "Aujourd'hui"
  const [todaySelectedDate, setTodaySelectedDate] = useState(new Date().toISOString().split('T')[0]);
  const [showDatePickerToday, setShowDatePickerToday] = useState(false);
  ...
  const loadSlots = async () => {
    try {
      ...
      if (savedSlots) {
        ...
        setTimeSlots(parsed);
        if (parsed.length > 0 && !boatSlot) {
          setBoatSlot(parsed[0]);
        }
      } else {
        const defaultSlots = ['09h-11h', '11h-13h', '13h-15h', '15h-17h', '17h-19h'];
        setTimeSlots(defaultSlots);
        setBoatSlot(defaultSlots[0]);
        await AsyncStorage.setItem('timeSlots', JSON.stringify(defaultSlots));
      }
    } catch (error) {
  ...
  const calculerReste = () => {
    ...
    const reste = Math.max(0, total - vers);
    setResteAPayer(reste.toString());
  };

  const calculerResteBateau = () => {
    const total = parseFloat(boatTotalAPayer) || 0;
    const vers = parseFloat(boatVersement) || 0;
    const reste = Math.max(0, total - vers);
    setBoatResteAPayer(reste.toString());
  };

  // Récupérer les heures de début des créneaux
  const getBoatStartTimes = () => {
    const startTimes = [];
    timeSlots.forEach(slot => {
      const [start, end] = slot.split('-');
      const startHour = parseInt(start.replace('h', ''));
      startTimes.push(`${startHour.toString().padStart(2, '0')}:00`);
    });
    return startTimes;
  };

  // Vérifier la disponibilité d'un créneau bateau
  const checkBoatAvailability = (date, startTime, boatNum, durationMinutes) => {
    ...
    return boatReservations.some(r => {
      ...
      if (r.bateau !== `Bateau ${boatNum}`) return false;
      
      const rStartTotal = timeToMinutes(r.heure);
      const rDuration = parseInt(r.duree) || 60;
      const rEndTotal = rStartTotal + rDuration;
      
      return (startTotal < rEndTotal && endTotal > rStartTotal);
    });
  };
  ...
  const getActivityColor = (activite) => {
    const colors = {
      ...
      'Parapente': '#9c27b0',
      'Kayak': '#ff6f00',
      'Bouée tractée': '#e91e63'
    };
    return colors[activite] || '#757575';
  };

  // ===== FONCTIONS DE RÉSERVATION AVEC PAIEMENT =====
  ...
  const addReservation = async () => {
    ...
    const vers = parseFloat(versement) || 0;
    if (vers > total) {
      Alert.alert('Erreur', 'Le versement ne peut pas dépasser le total à payer');
      return;
    }
  ...
  const addBoatReservation = async () => {
    ...
    if (!boatSelectedTime) {
      Alert.alert('Erreur', 'Veuillez sélectionner un créneau horaire en cliquant dessus');
      return;
    }
    const total = parseFloat(boatTotalAPayer) || 0;
    if (total <= 0) {
    ...
    const durationMinutes = parseInt(boatCircuit);
    const startTime = boatSelectedTime;

    if (checkBoatAvailability(boatDate, startTime, boatNumber, durationMinutes)) {
      Alert.alert('Erreur', `Ce créneau est déjà réservé pour cette date et le bateau ${boatNumber} !`);
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setBoatReservations(updatedReservations.filter(r => r.activite === 'Bateau'));
        saveReservationsToLocal(updatedReservations);
        Alert.alert('Succès', `Réservation confirmée !\nBateau ${boatNumber} - ${boatDate} à ${startTime}\nDurée: ${durationMinutes} min\nTotal: ${total} DA - Versé: ${vers} DA - Reste: ${Math.max(0, total - vers)} DA`);
        resetBoatForm();
      } else {
        throw new Error('Erreur serveur');
      }
  ...
  const resetClientForm = () => {
    setNom(''); setPrenom(''); setTel(''); setPersonnes('1'); setHeure(''); setDate('');
    setTotalAPayer(''); setVersement(''); setResteAPayer('0'); setNote('');
    setModalVisible(false);
  };

  const resetBoatForm = () => {
    setBoatNom(''); setBoatPrenom(''); setBoatTel(''); setBoatDate('');
    setBoatTotalAPayer(''); setBoatVersement(''); setBoatResteAPayer('0'); setBoatNote('');
    setBoatSelectedTime('');
    setModalVisible(false);
  };
  ...
  const renderTodayReservations = () => {
    const displayedReservations = getReservationsByDate(todaySelectedDate);
    const displayedBoatReservations = displayedReservations.filter(r => r.activite === 'Bateau');
    
    return (
      <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }], flex: 1 }}>
        <View style={styles.todayHeader}>
          <Text style={styles.todayTitle}>📅 Réservations</Text>
          
          <TouchableOpacity style={styles.datePickerBtn} onPress={() => setShowDatePickerToday(true)}>
            <Ionicons name="calendar-outline" size={20} color="#666" />
            <Text style={styles.datePickerText}>📅 {todaySelectedDate}</Text>
          </TouchableOpacity>
          {showDatePickerToday && (
            <DateTimePicker
              value={new Date(todaySelectedDate)}
              mode="date"
              display="default"
              onChange={(event, selectedDate) => {
                setShowDatePickerToday(false);
                if (selectedDate) {
          ...
          <Text style={styles.todayDate}>
            {new Date(todaySelectedDate).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
          </Text>
          <Text style={styles.todayCount}>{displayedReservations.length} réservation(s)</Text>
        </View>
        ...
        {reservationsLoading ? (
          ...
          <>
            {displayedBoatReservations.length > 0 && (
              <View style={styles.boatTodaySection}>
                <Text style={styles.boatTodayTitle}>🚤 Réservations Bateau</Text>
                {timeSlots.map(slot => {
                  const slotsInSlot = displayedBoatReservations.filter(r => r.slot === slot);
                            ...
                            <View key={boatNum} style={styles.boatTodayCell}>
                              ...
                              {boatRes.map((r, idx) => (
                                <TouchableOpacity key={idx} style={styles.boatTodayReservation} onPress={() => {
                                  ...
                                  <Text style={styles.boatTodayTime}>⏰ {r.heure}</Text>
                                  ...
                                  <Text style={[styles.boatTodayPayment, styles.boatTodayReste]}>📊 Reste: {r.resteAPayer || 0} DA</Text>
                                  {r.note && r.note !== 'Aucune note' ? (
                                    <Text style={styles.boatTodayNote}>📝 {r.note}</Text>
                                  ) : null}
                                </TouchableOpacity>
                              ))}
                            </View>
            ...
            <View style={styles.todayTableContainer}>
              <View style={styles.tableHeader}>
                ...
                <Text style={[styles.tableHeaderCell, styles.cellVersement]}>Versé</Text>
                <Text style={[styles.tableHeaderCell, styles.cellReste]}>Reste</Text>
                <Text style={[styles.tableHeaderCell, styles.cellPayment]}>Statut</Text>
              </View>
              {displayedReservations.filter(r => r.activite !== 'Bateau').map((reservation, index) => {
                const payment = getPaymentStatusText(reservation.paymentStatus || 'non_paye');
                const total = reservation.totalAPayer || 0;
                const vers = reservation.versement || 0;
                const reste = reservation.resteAPayer || 0;
                return (
                  <TouchableOpacity key={reservation.id || index} style={styles.tableRow} onPress={() => {
                    ...
                    <Text style={[styles.tableCell, styles.cellTotal]}>{total} DA</Text>
                    <Text style={[styles.tableCell, styles.cellVersement]}>{vers} DA</Text>
                    <Text style={[styles.tableCell, styles.cellReste, reste > 0 ? styles.restePositive : styles.resteZero]}>
                      {reste} DA
                    </Text>
              ...
              {displayedReservations.length === 0 && (
                <View style={styles.emptyState}>
                  ...
                  >
                    ...
                    <Text style={styles.emptyStateText}>Aucune réservation pour cette date</Text>
                    <Text style={styles.emptyStateSubtext}>Sélectionnez une autre date</Text>
                  </LinearGradient>
                </View>
              )}
            </View>
      ...
      <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
        ...
        {reservationsLoading ? (
          ...
          filteredReservations.map((r, index) => {
            ...
            const total = r.totalAPayer || 0;
            const vers = r.versement || 0;
            const reste = r.resteAPayer || 0;
            return (
                  ...
                  >
                    ...
                    <View style={styles.reservationInfo}>
                      ...
                      <View style={styles.reservationRow}>
                        ...
                        <Text style={styles.reservationValue}>💰 Total: {total} DA</Text>
                      </View>
                      <View style={styles.reservationRow}>
                        <Ionicons name="card-outline" size={16} color="#666" />
                        <Text style={styles.reservationValue}>💳 Versé: {vers} DA</Text>
                      </View>
                      <View style={styles.reservationRow}>
                        <Ionicons name="trending-down-outline" size={16} color="#666" />
                        <Text style={[styles.reservationValue, reste > 0 ? styles.restePositive : styles.resteZero]}>
                          📊 Reste: {reste} DA
                        </Text>
                      </View>
                      <View style={styles.reservationRow}>
  ...
  const renderBoatTab = () => {
    const startTimes = getBoatStartTimes();
        ...
        <View style={styles.boatFormCard}>
          ...
          <View style={styles.formBody}>
            ...
            <TouchableOpacity style={styles.datePickerBtn} onPress={() => setShowBoatDatePicker(true)}>
              <Ionicons name="calendar-outline" size={20} color="#666" />
              <Text style={styles.datePickerText}>{boatDate ? `📅 Date: ${boatDate}` : '📅 Sélectionner une date'}</Text>
            </TouchableOpacity>
            {showBoatDatePicker && (
              <DateTimePicker
                value={boatDate ? new Date(boatDate) : new Date()}
                mode="date"
                display="default"
                onChange={(event, selectedDate) => {
            ...
            <View style={styles.pickerContainer}>
              ...
              <View style={styles.pickerButtons}>
                {['60', '90', '120', '150', '180', '210'].map(d => (
                  <TouchableOpacity key={d} style={[styles.pickerOption, boatCircuit === d && styles.pickerOptionSelected]} onPress={() => {
                    setBoatCircuit(d);
                    const price = BOAT_PRICE_BY_MINUTES[d];
                    if (price) {
                    ...
                    <Text style={[styles.pickerOptionText, boatCircuit === d && styles.pickerOptionTextSelected]}>{parseInt(d) / 60}h</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
            ...
            <View style={styles.pickerContainer}>
              <Text style={styles.pickerLabel}>Choisissez un créneau:</Text>
              <View style={styles.boatSlotsContainer}>
                {startTimes.map((time, idx) => {
                  const isAvailable = boatDate ? !checkBoatAvailability(boatDate, time, boatNumber, parseInt(boatCircuit)) : false;
                  const isSelected = boatSelectedTime === time;
                  ...
                  return (
                    <TouchableOpacity 
                      ...
                      style={[
                        styles.boatSlotItem,
                        isAvailable ? styles.boatSlotAvailable : styles.boatSlotUnavailable,
                        isSelected && styles.boatSlotSelected
                      ]}
                      disabled={!isAvailable || !boatDate}
                      onPress={() => {
                        if (!boatDate) {
                    ...
                    >
                      <Text style={[
                        styles.boatSlotText,
                        isAvailable ? styles.boatSlotTextAvailable : styles.boatSlotTextUnavailable,
                        isSelected && styles.boatSlotTextSelected
                      ]}>
                        {time}
                      </Text>
                      {!isAvailable && boatDate && (
                        <View style={styles.boatSlotBarre}>
                          <Text style={styles.boatSlotBarreText}>❌ Occupé</Text>
                        </View>
                      )}
                      {isSelected && isAvailable && (
                        <View style={styles.boatSlotCheck}>
                          <Text style={styles.boatSlotCheckText}>✅ Sélectionné</Text>
                        </View>
                      )}
                      {isAvailable && !isSelected && boatDate && (
                        <View style={styles.boatSlotFree}>
                          <Text style={styles.boatSlotFreeText}>✓ Libre</Text>
                        </View>
                      )}
                      {!boatDate && (
                        <View style={styles.boatSlotDisabled}>
                          <Text style={styles.boatSlotDisabledText}>? Choisir date</Text>
                        </View>
                      )}
                    </TouchableOpacity>
                  );
              ...
              {boatSelectedTime && (
                <Text style={styles.selectedTimeText}>⏰ Créneau sélectionné: {boatSelectedTime}</Text>
              )}
              {boatDate && !boatSelectedTime && (
                <Text style={styles.selectHintText}>👆 Cliquez sur un créneau libre</Text>
              )}
            </View>
            ...
            <View style={styles.pickerContainer}>
              ...
              <View style={styles.pickerButtons}>
                {['1', '2'].map(b => (
                  <TouchableOpacity key={b} style={[styles.pickerOption, boatNumber === b && styles.pickerOptionSelected]} onPress={() => {
                    setBoatNumber(b);
                    setBoatSelectedTime('');
                  }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={styles.modalBody}>
              ...
              <View style={styles.pickerContainer}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  <View style={styles.pickerButtons}>
                    {['Bateau', 'Visite guidée', 'Quad', 'Cheval', 'Jetski', 'Parapente', 'Kayak', 'Bouée tractée'].map(act => (
              ...
              <TouchableOpacity style={styles.datePickerBtn} onPress={() => setShowTimePicker(true)}>
                ...
                <Text style={styles.datePickerText}>{heure ? `⏰ Heure: ${heure}` : '⏰ Sélectionner une heure'}</Text>
              </TouchableOpacity>
              {showTimePicker && (
                <DateTimePicker
                  value={heure ? new Date(`2000-01-01T${heure}:00`) : new Date()}
                  mode="time"
                  display="default"
              ...
              <TouchableOpacity style={styles.datePickerBtn} onPress={() => setShowDatePickerClient(true)}>
                <Ionicons name="calendar-outline" size={20} color="#666" />
                <Text style={styles.datePickerText}>{date ? `📅 Date: ${date}` : '📅 Sélectionner une date'}</Text>
              </TouchableOpacity>
              {showDatePickerClient && (
                <DateTimePicker
                  value={date ? new Date(date) : new Date()}
                  mode="date"
                  display="default"
                  onChange={(event, selectedDate) => {
                    setShowDatePickerClient(false);
  ...
  const renderCreateEmployeurModal = () => {
    return (
                ...
                <ScrollView style={styles.detailBody}>
                  ...
                  <View style={styles.detailSection}>
                    ...
                    <View style={styles.detailRow}>
                      ...
                      <Text style={styles.detailLabel}>Versement:</Text>
                      <Text style={styles.detailValue}>{selectedReservation.versement || 0} DA</Text>
                    </View>
                    <View style={styles.detailRow}>
                      <Ionicons name="trending-down-outline" size={20} color={selectedReservation.resteAPayer > 0 ? '#c62828' : '#4caf50'} />
                      <Text style={styles.detailLabel}>Reste à payer:</Text>
                      <Text style={[styles.detailValue, selectedReservation.resteAPayer > 0 ? styles.restePositive : styles.resteZero]}>
                        {selectedReservation.resteAPayer || 0} DA
                      </Text>
                    </View>
                    <View style={styles.detailRow}>
...
const styles = StyleSheet.create({
  ...
  todayCount: { fontSize: 13, color: '#0077b6', textAlign: 'center', marginTop: 4, fontWeight: 'bold' },
  ...
  boatTodayPayment: { fontSize: 9, color: '#333', marginTop: 1 },
  boatTodayReste: { fontWeight: 'bold', color: '#c62828' },
  boatTodayNote: { fontSize: 9, color: '#666', marginTop: 2, fontStyle: 'italic' },
  ...
  tableHeaderCell: { color: 'white', fontWeight: 'bold', fontSize: 10 },
  ...
  cellClient: { flex: 1.5 },
  cellPhone: { flex: 1 },
  cellActivity: { width: 60 },
  cellPeople: { width: 30, textAlign: 'center' },
  cellTotal: { width: 45, textAlign: 'center' },
  cellVersement: { width: 45, textAlign: 'center' },
  cellReste: { width: 45, textAlign: 'center' },
  cellPayment: { width: 60, alignItems: 'center' },
  paymentCellBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 4, paddingVertical: 2, borderRadius: 8, gap: 2 },
  paymentCellText: { color: 'white', fontSize: 8, fontWeight: 'bold' },
  activityBadge: { paddingHorizontal: 4, paddingVertical: 2, borderRadius: 6, alignSelf: 'flex-start' },
  activityBadgeText: { color: 'white', fontSize: 8, fontWeight: 'bold' },
  restePositive: { color: '#c62828', fontWeight: 'bold' },
  resteZero: { color: '#4caf50', fontWeight: 'bold' },
  ...
  boatSlotsContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 5 },
  boatSlotItem: { 
    padding: 12, 
    borderRadius: 12, 
    borderWidth: 2, 
  ...
  boatSlotTextAvailable: { color: '#2e7d32' },
  boatSlotTextUnavailable: { color: '#c62828' },
  boatSlotTextSelected: { color: '#0077b6' },
  boatSlotDisabledText: { color: '#9e9e9e', fontSize: 10 },
  boatSlotBarre: { 
    marginTop: 4,
  ...
  boatSlotBarreText: { color: 'white', fontSize: 10, fontWeight: 'bold' },
  boatSlotCheck: { 
    marginTop: 4,
  ...
  boatSlotCheckText: { color: 'white', fontSize: 10, fontWeight: 'bold' },
  boatSlotFree: { 
    marginTop: 4,
  ...
  boatSlotFreeText: { color: 'white', fontSize: 10, fontWeight: 'bold' },
  selectedTimeText: { marginTop: 8, fontSize: 14, fontWeight: 'bold', color: '#0077b6', textAlign: 'center' },
  selectHintText: { marginTop: 8, fontSize: 12, color: '#666', textAlign: 'center', fontStyle: 'italic' },
  paymentContainer: { backgroundColor: '#f0f7ff', padding: 15, borderRadius: 12, borderWidth: 2, borderColor: '#0077b6', marginVertical: 10 },
  paymentTitle: { fontWeight: 'bold', color: '#0077b6', fontSize: 16, marginBottom: 10 },
  paymentRow: { flexDirection: 'row', gap: 10 },
  paymentField: { flex: 1 },
  paymentFieldFull: { flex: 1 },
  paymentLabel: { fontSize: 12, color: '#0077b6', marginBottom: 4 },
  paymentLabelReste: { color: '#c62828', fontWeight: 'bold' },
  paymentInput: { backgroundColor: 'white', borderWidth: 1, borderColor: '#0077b6', borderRadius: 8, padding: 10, fontSize: 14 },
  paymentInputReste: { backgroundColor: '#fff3e0', borderColor: '#f44336', fontWeight: 'bold', color: '#c62828' },
  noteInput: { minHeight: 60, textAlignVertical: 'top', marginTop: 4 },
